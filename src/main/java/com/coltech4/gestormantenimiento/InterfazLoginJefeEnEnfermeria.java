...
public class InterfazLoginJefeEnEnfermeria extends javax.swing.JFrame {
    ...
    public InterfazLoginJefeEnEnfermeria() {
    ...
    if (BaseDatos.jefeenfermeria.isEmpty()) {
        JOptionPane.showMessageDialog(
            rootPane,
            "No se ha registrado ningún enfermera.",
            "Advertencia",
            JOptionPane.WARNING_MESSAGE
        );
    ...
    // Buscar coincidencia
    for (i = 0; i < BaseDatos.jefeenfermeria.size(); i++) {
        if (BaseDatos.jefeenfermeria.get(i).documento == Integer.parseInt(CampoDocumento.getText()) &&
            BaseDatos.jefeenfermeria.get(i).nickname.equals(CampoNickname.getText())) {
            ...
            this.BaseDatos.MostrarOpcionesJefeDeEnfermeria();
